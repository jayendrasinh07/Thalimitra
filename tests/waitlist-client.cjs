...
assert.match(selectorSource, /setCurrentStep\('saved_success'\)/);
assert.match(selectorSource, /Address saved/);
assert.match(selectorSource, /activeTab === 'order_once' \? 'Continue checkout' : 'See menu & prices'/);
assert.match(selectorSource, /We will recheck the menu, cutoff and capacity before payment/);
...
console.log('PASS: private exact-area discovery, post-save continuation, cloud waitlist and admin parsing');
