...
assert.match(publicSource, /status: 'available' \| 'coming_soon'/);
...
assert.match(selectorSource, /comingSoonAreas/);
assert.match(selectorSource, /These routes are published for expansion/);
const coverageSource = readFileSync('src/pages/GandhinagarCoveragePage.tsx', 'utf8');
assert.match(coverageSource, /listPublicDeliveryAreas/);
assert.match(coverageSource, /Available now/);
assert.match(coverageSource, /Coming soon/);
