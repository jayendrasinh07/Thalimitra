...
export const NotificationBell = ({ compact = false }: { compact?: boolean }) => {
  ...
  return <button type="button" aria-label={hasUnread ? `Notifications, ${count} unread` : 'Notifications'} onClick={() => setActiveTab('notifications')}
    className={`group relative grid shrink-0 place-items-center rounded-full border outline-none transition duration-200 ease-out focus-visible:ring-2 focus-visible:ring-[#0D6E44] focus-visible:ring-offset-2 active:scale-95 ${compact ? 'h-11 w-11' : 'h-12 w-12'} ${hasUnread ? 'border-[#0D6E44] bg-[#0D6E44] text-white shadow-[0_6px_16px_rgba(13,110,68,0.22)] hover:bg-emerald-800' : 'border-transparent bg-stone-100 text-stone-700 hover:bg-emerald-100 hover:text-[#0D6E44]'} ${hasNewActivity ? 'motion-safe:scale-110' : ''}`}>
