  ...
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-stone-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="subscription-title">
    <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
      <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-emerald-800/30 bg-[#0D6E44] p-5 text-white sm:p-6">
        <div><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-300">No payment now</p><h2 id="subscription-title" className="mt-1 text-2xl font-black">Request a meal plan</h2><p className="mt-1 text-sm text-emerald-100">Choose your meals and address. We confirm the price before you pay.</p></div>
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        <label className="block text-sm font-black text-stone-900">Anything else we should know? <span className="font-normal text-stone-500">Optional</span><textarea value={note} onChange={event => setNote(event.target.value)} maxLength={500} rows={3} placeholder="Preferred days or delivery details we should consider." className="mt-2 w-full rounded-xl border border-stone-200 p-3 text-sm font-normal" /></label>

        {error && <p role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-800">{error}</p>}
        <div className="rounded-2xl bg-stone-50 p-4 text-xs leading-5 text-stone-600"><p className="flex items-start gap-2"><ShieldCheck className="h-4 w-4 shrink-0 text-emerald-700" /><span><strong className="text-stone-900">No automatic charge.</strong> We check your address, schedule and total first. The plan starts only after payment is verified.</span></p></div>
