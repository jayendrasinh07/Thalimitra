...
const OfferEditor = ({ draft, document, advanced, busy, onAdvanced, onChange, onCancel, onSave, toggle }: {
  ...
  return <section className="rounded-3xl border border-emerald-200 bg-white p-5 shadow-lg sm:p-6">
    ...
    {advanced && <div className="mt-4 space-y-5 rounded-2xl border border-stone-200 p-4">
      ...
      <Scope title="Specific dishes" hint="None selected means every eligible dish.">{document?.meals.map(meal => <div key={meal.id}><ScopeOption checked={draft.eligibleMealIds.includes(meal.id)} label={`${meal.name} · ${meal.meal_type}`} onChange={() => set('eligibleMealIds', toggle(draft.eligibleMealIds, meal.id))} /></div>)}</Scope>
      <Scope title="Specific delivery areas" hint="None selected means every available area.">{document?.areas.map(area => <div key={area.id}><ScopeOption checked={draft.eligibleZoneIds.includes(area.id)} label={`${area.name} · ${area.status}`} onChange={() => set('eligibleZoneIds', toggle(draft.eligibleZoneIds, area.id))} /></div>)}</Scope>
    </div>}
