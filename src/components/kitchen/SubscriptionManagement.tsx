  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-5 space-y-4">{items.map(item => <article key={item.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black capitalize ring-1 ring-stone-200">{item.status.replace('_',' ')}</span><span className="text-xs font-bold capitalize text-emerald-700">{item.meal_types.join(' + ')}</span></div><h3 className="mt-2 text-lg font-black text-stone-900">{item.customer_name} · {item.plan_name}</h3><p className="mt-1 text-xs text-stone-500">Requested {date(item.created_at)} · Start {item.preferred_start_date}</p></div><div className="text-right"><p className="text-sm font-black text-stone-900">{item.quoted_total == null ? 'Quote not set' : money(item.quoted_total)}</p><p className="text-xs font-bold capitalize text-stone-500">Payment {item.payment_status}</p></div></div>
