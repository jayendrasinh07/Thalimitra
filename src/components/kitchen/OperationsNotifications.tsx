...
export const OperationsNotifications = ({ onNavigate }: { onNavigate: (target: string) => void }) => {
  ...
  return <div className="relative print:hidden">
    <button type="button" onClick={() => setOpen(value => !value)} aria-expanded={open} aria-label={unread ? `Operations alerts, ${unread} unread` : 'Operations alerts'} className={`group relative grid h-11 w-11 place-items-center rounded-full border shadow-sm outline-none transition duration-200 focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-2 active:scale-95 ${open || unread > 0 ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-stone-200 bg-white text-stone-600 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800'}`}>
